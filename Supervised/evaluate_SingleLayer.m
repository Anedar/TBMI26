...
% The Training Data
Xtraining = [ones(1, length(Xt{1})); Xt{1}]; %% Ok vektorform?
% Dtraining = [ones(1, length(Xt{1})); Dt{1}];

Xtest = [ones(1, length(Xt{2})); Xt{2}]; %% Samma som ovan i guess, kolla om det är ok.
%Dtest = [ones(1, length(Dt{2})); Dt{2}];
    ...
    learningRate = 0.0000005; % Change this, Your learningrate
    W0 = rand(size(Dt{1}, 1), size(Xtraining, 1))/50 - 0.01; % Change this, Initiate your weight matrix W W
